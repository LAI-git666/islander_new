...
class SmartNPC:
    ...
    def _normalize_resource(self, resource_type: str) -> str:
        ...
        return aliases.get(resource_type.strip(), resource_type.strip())
    ...
    def gather_resource(self, resource: Dict, world):
        ...
        # 记录到互动记录
        self.dialog_system.add_action_event(
            self.name,
            "gather",
            f"拾取了{gather_amount}个{resource_info['gather']}"
        )
    ...
    def eat(self):
        ...
        if self.inventory["果实"] > 0:
            ...
            self.dialog_system.add_action_event(self.name, "eat", "吃了1个果实，恢复了能量")
            ...
            self.dialog_system.add_action_event(self.name, "eat", "吃了1条鱼，恢复了能量")
    ...
    def drink(self):
        ...
        if self.inventory["水"] > 0:
            ...
            self.dialog_system.add_action_event(self.name, "drink", "喝了1份水，恢复了能量")
    ...
    def give(self, target_npc: 'SmartNPC', resource_type: str, amount: int):
        ...
        # 统一资源名称，避免“fruit”之类的英文导致判定失败
        resource_type = self._normalize_resource(resource_type)
        ...
        self.dialog_system.add_action_event(
            self.name,
            "give",
            f"{max_possible}个{resource_type}",
            target_npc.name
        )
    ...
    def interact_with_nearby_npcs(self):
        ...
        if self.nearby_npcs:
            ...
            available_npcs = [npc for npc in self.nearby_npcs]
...
请继续对话，保持自然。"""
    ...
    def decide_action(self, world_state: str) -> Dict:
        ...
        current_time = time.time()
        # 使用独立的AI决策间隔，不依赖FPS
        if current_time - self.last_action_time > AI_DECISION_INTERVAL:
