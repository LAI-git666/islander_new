...
class UIRenderer:
        ...
        if not conversations:
            ...
            for npc1, npc2, message, timestamp in conversations[-8:]:
                ...
                if len(message) <= max_chars:
                    # 短消息直接显示
                    if y_offset < 280:
                        msg_text = self.tiny_font.render(message, True, (255, 255, 255))
    ...
    def draw_communication_events(self, screen, dialog_system):
        ...
        actions = dialog_system.get_recent_actions(8)
        y_offset = 360

        if not actions:
            no_action_text = self.small_font.render("暂无互动记录", True, (150, 150, 150))
            screen.blit(no_action_text, (30, 370))
        else:
            for action in reversed(actions):
                ...
                if action_type == "gather":
                    display_text = f"{npc_name}: {details}"
                elif action_type == "give" and target:
                    display_text = f"{npc_name}赠送给{target}: {details}"
                elif action_type == "eat":
                    display_text = f"{npc_name}: {details}"
                elif action_type == "drink":
                    display_text = f"{npc_name}: {details}"
                elif action_type == "thought":
                    display_text = f"{npc_name}(思考): {details}"
                else:
                    display_text = f"{npc_name}: {details}"

                header_text = self.tiny_font.render(
                    f"{display_text[:35]}... ({time_str})" if len(display_text) > 35 else f"{display_text} ({time_str})",
                    True, (200, 200, 100)
                )
